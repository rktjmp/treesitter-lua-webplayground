...
window.translate = function (input_el, output_el) {
  Parser.init().then(async () => {
    ...
    const lua = await Parser.Language.load("tree-sitter-lua.wasm");
    parser.setLanguage(lua);
    const sourceCode = input;

    const tree = parser.parse(sourceCode);
    console.log("tr-parse: ", tree.rootNode);

    output_el.value = tree.rootNode.toString()
...
window.translate(document.getElementById("input"), document.getElementById("output"))
